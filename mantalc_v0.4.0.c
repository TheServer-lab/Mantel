/*
 * Mantel Compiler v0.4.0 - Enhanced Edition
 * New features: Comments, len(), input(), read_int(), array literals
 * Fixed version with complete functionality and bug fixes
 */
...
void gen_var(CodeGen *cg, Node *node) {
    // Simple variable access - assume it's on stack at rbp-8
...
void gen_call(CodeGen *cg, Node *node) {
    ...
    if (cg->is_windows) {
        emit(cg, "    sub rsp, 32");
        if ((node->call.arg_count > 4) && ((node->call.arg_count - 4) % 2 == 1)) {
            emit(cg, "    sub rsp, 8");
        }
    }
...
void gen_function(CodeGen *cg, Node *node) {
    ...
    /* Single local slot init */
    emit(cg, "    mov qword [rbp-8], 0");
...
int main(int argc, char **argv) {
    if (argc < 2) {
        printf("Mantel Compiler v0.4.0\n");
        printf("Enhanced with comments, builtin functions, and better features!\n\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            printf("Mantel Compiler v0.4.0 - Enhanced Programming Language\n");
            printf("Features: Functions, Variables, Loops, Conditionals, Printing,\n");
            printf("          Arrays, Strings, Comments, Input, len(), Type Inference\n\n");
            return 0;
        }
    }

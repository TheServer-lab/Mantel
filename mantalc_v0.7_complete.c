...
typedef enum {
    ...
    NODE_BINOP, NODE_UNOP, NODE_POSTFIX, NODE_CALL, NODE_CAST,
...
typedef struct {
    ...
    char *loop_end_labels[32];
    char *loop_continue_labels[32];
    int loop_depth;
} CodeGen;
...
Node *parse_primary(Parser *p) {
    ...
    if (parser_match(p, TOK_ID)) {
        ...
        if (parser_match(p, TOK_INC) || parser_match(p, TOK_DEC)) {
            TokenType op_type = p->current.type;
            parser_next(p); /* consume ++ or -- */
            
            Node *postfix = create_node(NODE_POSTFIX, current_node->line, current_node->col);
            postfix->postfix.operand = current_node;
            postfix->postfix.op = (op_type == TOK_INC) ? '+' : '-';
            current_node = postfix;
        }
...
void push_loop_labels(CodeGen *cg, char *end_label, char *continue_label) {
    if (cg->loop_depth < 32) {
        cg->loop_end_labels[cg->loop_depth] = end_label;
        cg->loop_continue_labels[cg->loop_depth] = continue_label;
        cg->loop_depth++;
    }
}

void pop_loop_labels(CodeGen *cg) {
    if (cg->loop_depth > 0) {
        cg->loop_depth--;
    }
}

void gen_break(CodeGen *cg, Node *node) {
    if (cg->loop_depth > 0) {
        emit(cg, "    jmp %s", cg->loop_end_labels[cg->loop_depth - 1]);
    } else {
        fprintf(stderr, "Error: break statement outside of loop\n");
    }
}

void gen_continue(CodeGen *cg, Node *node) {
    if (cg->loop_depth > 0) {
        emit(cg, "    jmp %s", cg->loop_continue_labels[cg->loop_depth - 1]);
    } else {
        fprintf(stderr, "Error: continue statement outside of loop\n");
    }
}

void gen_unop(CodeGen *cg, Node *node) {
    if (node->unop.op == '!') {
        /* Logical NOT */
        gen_expr(cg, node->unop.operand);
        emit(cg, "    cmp rax, 0");
        emit(cg, "    sete al");
        emit(cg, "    movzx rax, al");
    } else if (node->unop.op == '-') {
        /* Negation */
        gen_expr(cg, node->unop.operand);
        emit(cg, "    neg rax");
    } else if (node->unop.op == '~') {
...
void gen_postfix(CodeGen *cg, Node *node) {
    /* Post-increment (i++) or post-decrement (i--) */
    if (node->postfix.operand->type == NODE_VAR) {
        int offset = get_variable_offset(cg, node->postfix.operand->name);
        emit(cg, "    mov rax, [rbp%d]", offset);  /* Return old value */
        if (node->postfix.op == '+') {
            emit(cg, "    add qword [rbp%d], 1", offset);
        } else {
            emit(cg, "    sub qword [rbp%d], 1", offset);
        }
    }
}
...
void gen_while(CodeGen *cg, Node *node) {
    ...
    push_loop_labels(cg, end_label, loop_label);
...
void gen_for(CodeGen *cg, Node *node) {
    ...
    /* Push loop labels for break/continue */
    push_loop_labels(cg, end_label, inc_label);
...
void gen_expr(CodeGen *cg, Node *node) {
    ...
    switch (node->type) {
        ...
        case NODE_UNOP: gen_unop(cg, node); break;
        case NODE_POSTFIX: gen_postfix(cg, node); break;
        ...
        case NODE_BREAK: gen_break(cg, node); break;
        case NODE_CONTINUE: gen_continue(cg, node); break;
